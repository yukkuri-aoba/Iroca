...
2. 「ブラシで編集...」を押すとブラシパレットが開き、ペイントモードになります。
3. パレットの「マスクの種類」で除外／含めるを選びます（含めるはゾーン選択時のみ）。
4. プレビュー上をドラッグしてマスクを塗ります（赤い重ね表示が共通の除外、色付きがゾーン別の除外、緑が含める）。
5. 「塗る」「消す」でペイント／消しゴムを切り替えます。同じボタンをもう一度押すとペイントモードを抜けます。
...
3. 推定された領域は **その場で編集対象のマスクへ追加され**（既定は除外＝色替えしない範囲。「マスクの種類」で含めるに切り替え可能）、マスクの色で表示されます。確定ボタンはありません。パーツが複数の島に分かれている場合は、島を順に右クリックすればそれぞれが足されていきます。
