...
docs/booth/
  ...
  style.css    … デザイン（ブランドカラー・ロゴ・レイアウト）
  images/      … ここにスクショを置く（下の表のファイル名で）
  capture.py   … （任意）スライドを一括 PNG 化する補助スクリプト
  README.md    … このファイル
```

ロゴ・アイコン・カメレオンは CSS/SVG で描いているので、画像が無くても全スライドはそのまま完成します。
...
| スライド | ファイル名 | 推奨サイズ | 内容 |
|---|---|---|---|
| 1 ヒーロー | `images/hero.png` | 1600×900 (16:9) | メインビジュアル（任意。アバターや操作画面など） |
...
スライド 2・7・8 は文字と図解だけで完結するため、画像は不要です。
...
- 1 枚目（`slide-1` = 正方形ヒーロー）を **メイン画像** にすると、一覧サムネがきれいに揃います。
- 以降はスライド順（課題提起 → Before/After → 機能 → 技術 → 導入）に並べると流れが自然です。
...
- 色：`style.css` 冒頭の `:root` 変数（`--c-green` などのブランドカラー、`--grad-brand`）を変更。
