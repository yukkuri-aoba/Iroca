...
| # | 状態 | コミット | 備考 |
|---|------|---------|------|
| N-8 | **完了** | dev_safe `1605867` | 分岐の実態は「ahead 3 / behind 45」ではなく**ノート PC が 2026-07-01 の初期化コミットから分岐したまま 7 月分 45 コミットを一度も取り込んでいなかった**。統合により回帰テストが 13 本→30 本／381 ケースに回復（Feina/Quanstella の IoU・autotune、graymode 過検出、synth_gen2、AI マスク系、GT 生成スクリプト 12 本が丸ごと欠落していた） |
| N-3 | **完了** | dev_safe `052dbfc` | `.gitignore` のディレクトリ除外をやめ判定基準を追跡化。欠落 6 ベースラインを再生成（限界は `dev_safe/Tests/Baselines/README.md`）。skip 183→6、fail 6→0 |
| N-2 | **完了** | `cba0138` / dev_safe `4b88ff5` | `Code/Core/ZonesJsonDefaults.cs` へ単一ソース化＋スキーマ一致の機械検査。**指摘より影響範囲が広く**、出荷ゲートの視覚レビューまで非製品既定で生成されていた（→ N-9） |
| N-1 | **完了** | `bb635ed` / dev_safe `7530a0c` | ドナー収集と書き込みの両方に除外マスク参照を追加。**発火条件は指摘より狭い**（→ 下記） |
| N-5 | **完了** | `529d3a3` | リセットをジョブ中に無効化（ヘッダー全体ではなく、状態を破壊的に書き換えるボタンだけ）＋ オーバーレイ高を実測して `availableContentH` から予約 |
| N-7 | **完了** | `19e0735` | 実装（グローバル Undo）に合わせてラベル・ツールチップを是正。マスク限定 Undo の実装は Unity 標準 Undo 統合と衝突するため採らない |
...
```
VACC_CSHARP_GATE_FULL=1 pytest dev_safe/Tests/regression/ -q -ra
→ 398 passed, 6 skipped, 1 xfailed
（skip 6 はすべて VACC_FEINA_AUTOTUNE=1 の明示オプトイン。xfail は既知の WS-R 未実装）
```

### N-2 の対応で判明した補足

- N-2 の失敗シナリオは実測では**発生しなかった**。`useFloodFill` を製品既定（true）に
  揃えても回帰テストは 1 件も壊れていない。`fixtures._zone_to_harness` が全フィールドを
  明示書き出ししているためで、乖離が顕在化していたのは**手書き dict のテスト 5 本と
  視覚レビューだけ**だった。
- 視覚レビューへの影響は大きく、75 ケース中 70 ケースが変化した。ただし変化画素
  24,532,541 px のうち 82.2% は元テクスチャ方向への復帰（＝連結成分の外にあった誤爆着色の
  除去）で、GT に対する Recall は全被写体で不変、IoU は同等以上（feina-pants 0.092→0.107、
  誤爆 120 万画素が除去）。

### N-1 の対応で判明した補足 — 発火条件は指摘より狭い

書き込みは次を **全て** 満たす画素にだけ起きる: `strength==0` / マッチ画素が 2px 以内
(`mNear`) / 半径 4 に BG ドナー (`bgD`) / α が `[0.05, 0.70]` / 射影線からの距離が近い。

このため、失敗シナリオに書いた「マスクで守ったサンプル同色パーツ」は**そのままでは再現しない**。
地色一色の画素は α≈1 で「白拒否」ゲートに、背景色の画素は α<0.05 の下限に弾かれるためで、
実際 Feina 三角（無彩＋実マスク、`test_triangle_recolor_quality` と同じ配置）では**修正前でも
違反 0 件**だった。脆弱なのは **背景と地色の混色（保護パーツの AA 縁）が変換パーツの 2px 圏内に
ある** 場合のみ。この条件を厳密に満たす合成シーンで再現し（修正前: 除外画素 120 個が最大差 102 で
黒側へ移動）、`test_mask_contract_achroma_fringe.py` として固定した。契約違反であることに変わりは
ないが、実被写体での発生頻度は低い。
...
### N-9. 視覚レビューが zones JSON を過少指定していた [裏取り済み・N-2 で解消]

`tools/visual_review.py:108-116` は zones JSON に 7 フィールドしか書かず、残りをハーネス
既定に頼っていた。その結果、**出荷ゲートのパネルは製品でも回帰テスト設定でもない混成**を
映していた（`autoRecolorAnchor`=製品側 true / `useFloodFill`=回帰側 false）。
...
- `dev_safe/Tests/regression/fixtures.py:215-240` の docstring は
  ...
  と書くが、**連続領域モードは既に製品既定 ON**（`ColorZone.cs:166`、UI の標準）。
- 結果として **全回帰テストが製品と異なる選択挙動を測っている**。同 docstring が挙げる
  `highlight_recovery=True` / `shadow_desaturation=0.0` / `auto_recolor_anchor=False` も
  「Python 実装時代の既定」であり製品と異なる。当時の意図（エンジン差し替えの純粋な
  ドリフトだけを読む）は正当だが、移行完了後も据え置かれている。
- 影響: 製品既定でのみ現れる挙動（連結成分による絞り込み、`RecoverEnclosedNeutral` による
  閉領域復帰）が回帰テストの網の外にある。実際、N-2 の視覚レビューでは
  quanstella-eye の誤爆ブロッチ除去・haolan-hair の青残り解消という**製品既定でのみ観測できる
  改善**が見つかった。
...
生成せざるを得なかった。§6-22 の指摘（等倍クロップ列・差分ヒートマップが無い）は実務上の
ブロッカーとして実在する。`visual_review.py` に次を組み込むのが望ましい:
...
出荷ゲートを素通りし、修正後も全 75 ケースがビット一致で「何も検証できない」状態だった。
