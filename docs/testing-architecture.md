...
| 仕掛け | 何をしているか | 効かせない方法 |
|--------|----------------|----------------|
| 実行キャッシュ | `(DLL の中身, 入力, 除外マスク, zones JSON, 追加引数)` の SHA-256 をキーに出力 raw を `dev_safe/_harness_cache/` へ保存。同じ入力の再実行を丸ごと省く | `IROCA_HARNESS_CACHE=0` |
...
時点で全エントリが自動的に無効化される（古い出力で緑になることはない）。容量は既定 8GB 上限で
古い順に捨てる（`IROCA_HARNESS_CACHE_GB` で変更、`IROCA_HARNESS_CACHE_DIR` で移動）。

ハーネスのスレッド数は `IROCA_HARNESS_THREADS` で上書きできる（未設定＝製品と同じ
`ProcessorCount-2`）。並列ワーカーとの取り合いを調整したいときだけ使う。
