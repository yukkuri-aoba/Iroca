...
```powershell
# プロジェクトルートから
dotnet build scripts/build-check/VACCEditor.csproj
```
...
```powershell
# バージョンだけ変える
dotnet build scripts/build-check/VACCEditor.csproj -p:UnityVersion=2022.3.22f1

# フルパスを上書き（Hub 経由でないインストール等）
dotnet build scripts/build-check/VACCEditor.csproj -p:UnityEditorPath="D:\Unity\2022.3.22f1\Editor\Data\Managed"
