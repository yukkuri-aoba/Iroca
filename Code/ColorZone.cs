...
namespace Camereo
{
    ...
    public class ColorZone
    {
        ...
        // 複数スポイト（マルチサンプル選択）。空＝従来の単一サンプル挙動と完全に等価。
        // 同じパーツでも光の当たり方で色の濃淡（明部／暗部）があり、単一サンプル＋大きな
        // 許容範囲ではその濃淡を覆おうとして隣接パーツまで巻き込みやすい（誤爆の主因）。
        // そこでパーツの濃い所・薄い所を複数点スポイトし、各サンプルを中心とした
        // 小さな許容範囲の「和集合」でパーツ全体を捉える。これにより precision（はみ出し低減）と
        // recall（取りこぼし低減）を同時に高められる。
        //   sampleColor      … 主サンプル。再着色アンカー（＝出力色マッピング）に使う。
        //   extraSamples     … 追加スポイト。選択（マッチング）だけを広げ、出力色には影響しない。
        // 出力を主サンプル基準に固定するのは normalizeMatchSample / autoRecolorAnchor と同じ
        // 「選択と出力の分離」方針に沿う（追加サンプルで塗り分けがブレないようにするため）。
