    ...
    internal static class ZoneAutoTuner
    {
            ...
            bool[] excluded = null, int maskW = 0, int maskH = 0)
        {
            ...
            if (canAnalyze)
            {
                ...
                int extraN = zone.extraSamples?.Count ?? 0;
                if (extraN > 0)
                {
                    // ── マルチサンプル経路 ──────────────────────────────────────────
                    // 同じパーツの濃淡を複数点スポイトしたとき、tolerance は「各画素から
                    // 最も近いサンプルまでの距離」の高パーセンタイルで決める。サンプルが
                    // パーツのトーン範囲を覆っていれば、この最小距離はどこでも小さくなるので
                    // tolerance はタイトに収まり（precision 向上）、和集合でパーツ全体を覆える
                    // （recall 向上）。サンプルを増やすほど自然に tolerance が締まる。
                    var samples = BuildSampleHSVs(zone);
                    if (sampleS < AchromaSampleSatMax)
                    {
                        if (TryDeriveAchromaticToleranceMulti(pixels, width, height, samples,
                    ...
                    else if (TryDeriveChromaticToleranceMulti(pixels, width, height, zone, samples,
                                clusterMask, maskW, maskH, out float chromTol))
                    {
                        result.tolerance = chromTol;
                    }
                }
                else if (sampleS < AchromaSampleSatMax)
                {
        ...
        private static SampleHSV[] BuildSampleHSVs(ColorZone zone)
        {
            int extraN = zone.extraSamples?.Count ?? 0;
            var arr = new SampleHSV[1 + extraN];
            arr[0] = MakeSampleHSV(zone.sampleColor);
            for (int i = 0; i < extraN; i++) arr[i + 1] = MakeSampleHSV(zone.extraSamples[i]);
            return arr;
        }
        ...
        private static bool TryDeriveAchromaticToleranceMulti(Color32[] pixels, int w, int h,
            SampleHSV[] samples, bool[] excluded, int maskW, int maskH, out float tolerance)
        {
            tolerance = 0f;
            int stride = (w <= 2048) ? 1 : 2;
            var bins = new int[DistBins];
            ...
            for (int y = 0; y < h; y += stride)
            {
                ...
                for (int x = 0; x < w; x += stride)
                {
                    ...
                    float r = c.r / 255f, g = c.g / 255f, b = c.b / 255f;
                    Color.RGBToHSV(new Color(r, g, b, 1f), out _, out float pS, out float pV);
                    if (pS > AchromaClusterSatMax) continue; // 有彩は別素材として除外

                    float minDist = float.MaxValue;
                    bool inAnyNear = false;
                    for (int si = 0; si < samples.Length; si++)
                    {
                        var sm = samples[si];
                        if (Mathf.Abs(pV - sm.v) > AchromaVWindow) continue;
                        inAnyNear = true;
                        float dr = r - sm.r, dg = g - sm.g, db = b - sm.b;
                        float d = Mathf.Sqrt(dr * dr + dg * dg + db * db) * 0.57735027f;
                        if (d < minDist) minDist = d;
                    }
                    if (!inAnyNear) continue;
                    int bi = Mathf.Clamp((int)(minDist / DistMax * DistBins), 0, DistBins - 1);
