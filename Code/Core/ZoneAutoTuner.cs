...
namespace Iroca
{
    ...
    internal static class ZoneAutoTuner
    {
        ...
        //
        // 感度解析の結論:
        //   - 源色がほぼ全域で高彩度の素材(sS≈1.0): guard ON で過検出が大幅に減り IoU が改善。
        ...
        // 「導出 tolerance では拾わないはずの領域」が本番で選択される乖離があるため、
        // 最終判定は実マッチャーで検証する（Python プロキシ問題と同型の「モデルと実物の乖離」対策）。
        ...
        // 本物のスペキュラはパーツ面積の数%(実測: sneakers +1.0%、合成スペキュラ +2〜3%)、
        // 別素材の巻き込みは数十%以上(実測: 隣接する肌 +80%)なので明確に分離できる。
